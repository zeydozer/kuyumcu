        ...
        for (let j = 0; j < PRODUCT_TYPE.length; j++) {
          ...
          resp.carts.forEach((cart, i) => {
            ...
            rows +=
              ...
              if (cart.photo != null) subs.push(`<a href="#" onclick="fast.image('/img/cart/${cart.photo}'); return false"><i class="bi-image"></i> Fotoğraf</a>`)
              if (subs.length > 0) rows += `<div class="fst-cell-sub">${subs.join(' · ')}</div>`
              rows +=
                `</td>
                <td>${cart.width}</td>`
